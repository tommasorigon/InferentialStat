...
$$
\sqrt{n}(\hat{\theta}_n - \theta) \overset{d}{\longrightarrow} \text{N}(0, v(\theta)^{-1}), \qquad n \rightarrow \infty,
$$
...
$$
f(y; \theta) = \frac{1}{2}\phi(x; 0, 1) + \frac{1}{2}\phi(x; \theta, \exp\{-2/\theta^2\}),  \qquad y,\theta \in \mathbb{R},
$$
where $\phi(x; \mu, \sigma^2)$ is the [normal density]{.orange} with mean $\mu$ and variance $\sigma^2$, which satisfies conditions [(A1)-(A3)]{.blue}.
...
(TH2) (@Lehmann1998, Th. 3.7, Chap. 6) Under assumptions [(A1)-(A4)]{.blue}, suppose $\Theta \subseteq \mathbb{R}$, that $f(y; \theta)$ is [differentiable]{.blue} and that, with probability tending to one as $n\rightarrow \infty$, the score equation $\ell^*(\theta) = 0$ has a [unique solution]{.blue} $\hat{\theta}_n$. Then $\hat{\theta}_n$ is a [consistent estimator]{.orange} of $\theta_0$.

(TH3) (@Lehmann1998, Th. 5.1, Chap. 6) Under assumptions [(A1)-(A6)]{.blue}, suppose that, with probability tending to one as $n\rightarrow \infty$, the score equations $\ell^*(\theta) = \bm{0}$ have a [unique solution]{.blue} $\hat{\theta}_n$. Then $\hat{\theta}_n$ is a [consistent estimator]{.orange} of $\theta_0$.

## Comments of consistency results

- TH1 requires the [finiteness]{.orange} of $\Theta$, which is clearly [restrictive]{.orange}. 

- On the other hand, the statements of TH2 and TH3 are complicated by the possibility [multiple roots]{.blue}, so one must shown existence and uniqueness of $\hat{\theta}_n$ by other means. 
