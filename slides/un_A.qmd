...
Let $Y_1,\dots,Y_n$ be a iid sample from $f(y \mid \theta)\mathrm{d}y$ satisfying conditions C.1 and C.2, and let $\Theta = \mathbb{R}$. Moreover, let $\hat{\theta}(\bm{Y})$ be an [unbiased]{.orange} estimator of $\theta$  with finite variance $\text{var}(\hat{\theta}(\bm{Y})) < \infty$. Then
$$
\text{var}(\hat{\theta}(\bm{Y})) \ge \frac{1}{I(\theta)}.
$$
:::
