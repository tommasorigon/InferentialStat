...
An estimator $\hat{\theta}$ is a [best unbiased estimator]{.blue} of $\theta$ if it satisfies $\mathbb{E})\hat{\theta} = \theta$ for all $\theta$ and, for any other unbiased estimator $\tilde{\theta}$ 
::::
...
$$
\text{var}(\hat{\theta}(\bm{Y})) \ge \frac{[1 + \text{bias}'(\theta)]^2}{\mathbb{E}_\theta(l^*(\theta)^2)}.
$$
:::
