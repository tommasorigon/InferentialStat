...
## Bias reduction using Firth's corrections I
...
- Motivated by this, @Firth1993 proposed a [modified score equation]{.blue}. The [bias]{.orange} in $\hat{\theta}$ can be [reduced]{.orange} by introducing a [small bias]{.blue} into the [score function]{.blue}.  
...
$$
where $A(\theta)$ is any vector such that 
$$
\mathbb{E}_\theta(A(\theta)) = - I(\theta)\frac{b_1(\theta)}{n} + \mathcal{O}(n^{-1/2}).
$$
Clearly, a natural candidate for $A(\theta)$ is indeed $A(\theta) = - I(\theta)b_1(\theta)/n$.
:::

## Bias reduction using Firth's corrections II


![](firth.png){width=4in fig-align="center"}



## Bias reduction using Firth's corrections III
