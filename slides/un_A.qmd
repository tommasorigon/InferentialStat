...
:::callout-note
Let $Q(\theta)$ and $\tilde{Q}(\theta)$ be two [estimating equations]{.orange} with Godambe information $G(\theta)$ and $\tilde{G}(\theta)$. Then $Q(\theta)$ is [uniformly more efficient]{.blue} than $\tilde{Q}(\theta)$ if 
$$
G(\theta) \ge \tilde{G}(\theta) \qquad\text{ for all } \qquad \theta \in \Theta \subseteq \mathbb{R}.
$$
:::
...
:::callout-tip
Godambe information generalizes to the vector case $\theta \in \Theta \subseteq \mathbb{R}^p$ and is defined as $G(\theta) = H(\theta) J(\theta)^{-1}H(\theta)^T$, sometimes called [sandwich]{.orange} matrix. Optimality results generalize as well.
:::
...
$$
G(\theta) = \frac{\left(\sum_{i=1}^n w_i(\theta)\mu_i(\theta)\right)^2}{\sum_{i=1}^n w_i^2(\theta) V_i(\theta)}.
$$
...
$$
Consequently, the [Godambe information]{.blue} is $G(\beta) =  \sigma^{-2} (\bm{A}^T \bm{X})^T(\bm{A}^T \bm{A})^{-1}(\bm{A}^T \bm{X})^T$.
...
$$
G(\beta)^{-1} =  \sigma^2 \left[(\bm{A}^T \bm{X})^{-1}\bm{A}^T\right] \left[(\bm{A}^T \bm{X})^{-1}\bm{A}^T\right]^T.
$$

- The key remark is the following: a direct calculation shows that the variance of $\hat{\beta}$ is
$$
\text{var}(\beta) = \sigma^2 \left[(\bm{A}^T \bm{X})^{-1}\bm{A}^T\right] \left[(\bm{A}^T \bm{X})^{-1}\bm{A}^T\right]^T = G(\beta)^{-1},
$$
...
$$
\hat{\beta} = (\bm{X}^T\bm{X})^{-1}\bm{X}^T\bm{Y}, \qquad G(\beta) = \frac{1}{\sigma^2}\bm{X}^T\bm{X}.
$$

- Moreover, if  $\bm{Y} \sim \textup{N}_p(\bm{X}\beta, \sigma^2 I_p)$, then $G(\beta)$ also coincides with the [Fisher information]{.blue} $I(\beta)$. 
...
:::callout-tip
In regular estimation problems, the maximum likelihood estimator $\hat{\theta}$ is [asymptotically efficient]{.orange}, that is, for $n$ large enough, its variance attains the Cramér-Rao lower bound. Informally, 
$$
\hat{\theta} \:\dot{\sim}\: \text{N}(\theta, (n i(\theta))^{-1}).
$$
:::
...
$$
\sqrt{n}(\hat{\theta}_n - \theta_0) \overset{\text{d}}{\longrightarrow} \text{N}(0, i(\theta)^{-1}), \quad i(\theta) = -\int\frac{\partial}{\partial \theta \partial \theta^T}\log{f(y \mid \theta)}\mathrm{d}y,
$$
where $i(\theta)$ is the [Fisher information matrix]{.blue}.
:::
...
- For the [practical]{.blue} construction of [confidence intervals]{.orange}, or simply to empirically assess the variance of an estimator, we need to consider a consistent estimator for $I(\theta) = n i(\theta)$. 

:::callout-note
The negative [Hessian matrix]{.orange} of the log-likelihood is called [observed information]{.blue} and equals:
$$
j(\theta) := - \frac{\partial^2}{\partial \theta \partial \theta^T} \ell(\theta; \bm{Y}).
$$
If the second Bartlett identity holds, the [Fisher information]{.orange} is $I(\theta) = \mathbb{E}_\theta(j(\theta))$.
:::

- There are two natural candidates for estimating $I(\theta)$, namely $I(\hat{\theta}_n)$  and $j(\hat{\theta}_n)$. These two quantities may coincide, but in general it is not guaranteed.

- Following the original work of Fisher, @Efron1978 suggested using $j(\hat{\theta}_n)$ because it approximate the [conditional variance]{.blue} of $\hat{\theta}_n$ given an appropriate [ancillary statistics]{.orange}. 

- Besides, $j(\hat{\theta}_n)$ can be numerically computed via differentiation, whereas $i(\hat{\theta}_n)$ requires some analytical work. 

## Consistency for estimating equations

## Asymptotic bias and Firth corrections

- First order and second-order efficienct, corrections
