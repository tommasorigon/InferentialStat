...
[^invariance]: It generalizes to any $\psi(\cdot)$. If $\hat{\theta}$ is the MLE, then $\hat{\psi} = \psi(\hat{\theta})$ is the "MLE" of an "induced likelihood".
...
::: callout-tip
- We will get back to this example, as both $\hat{p}$ and $\hat{p}_\text{minimax}$ are [optimal]{.blue} according to some criteria.  

- In fact, $\hat{p}$ is the [UMVUE estimator]{.orange} (uniform minimum variance unbiased estimator), and $\hat{p}_\text{minimax}$ is the [minimax]{.orange} estimator.  
:::
...
- The risk function approach implicitly assumes the [repeatability of the experiment]{.blue}, which has sparked controversy on both philosophical and practical grounds. 
...
Any unique Bayes estimator is [admissible]{.blue}.  
:::
...
An estimator $\hat{\theta}$ is [unbiased]{.blue} if $\mathbb{E}_\theta(\hat{\theta}) = \theta$, that is, if $\text{bias}(\hat{\theta}) = \mathbb{E}_\theta(\hat{\theta} - \theta) = 0$ for all $\theta \in \Theta$.
:::
...
- To determine which estimator, $\bar{Y}$ or $S^2$, is preferable, we should [compare their variances]{.blue}. It is well known that $\text{var}_\lambda(\bar{Y}) = \lambda / n$, whereas computing $\text{var}_\lambda(S^2)$ can be [lengthy]{.orange}.  
...
[^mean_variance]: These are a basic and well-known results of inferential statistics. See Theorem 5.2.6 in @Casella2002.
...
- The Cramer-Rao theorem establishes a [lower bound]{.blue} for the variance of an estimator. Thus, if the variance of an unbiased estimator attains the lower bound for all $\theta$, then it is [optimal]{.orange}.
