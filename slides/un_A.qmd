...
- Note this does not contradict Wald inequality, because the spykes occur on a [set of measure zero]{.orange}. Indeed for any fixed $\theta$ the probability of observing $Y_i = \theta$ is zero.

- There are [multiple roots]{.blue} of the score equation $\ell^*(\theta)$, corresponding to the spykes and the "correct" solution around $\theta_0$. 
...
- The picture depict a possible $- \text{KL}(f(\cdot; \theta_0) \mid f(\cdot; \theta))$ an hypothetical situation in which the true value is $\theta_0 = \pi/2$. The presence of the asymptote may cause $\hat{\theta}_n$ to [diverge]{.orange}. 
...
:::callout-tip
The situation is not bad as it seems if we further require that, with probability tending to 1 as $n\to \infty$, there is a [unique solution]{.blue} to the [score equation]{.orange}. In that case $\hat{\theta}_n$ is the global maximizer of $\ell(\theta)$ and is also [consistent]{.orange}.
:::

[^th_vdv]: See also @vandervaart1998, Theorem 5.42, for slightly less stringent conditions and a careful discussion.
