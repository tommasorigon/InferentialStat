...
:::callout-tip
Godambe information generalizes to the vector case $\theta \in \Theta \subseteq \mathbb{R}^p$ and is defined as $V(\theta) = H(\theta) J(\theta)^{-1}H(\theta)^T$, sometimes called [sandwich]{.orange} matrix. Optimality results generalize as well.
:::
...
$$
for some $n \times p$ matrix $\bm{A}$ having full rank. This is a [linear unbiased]{.orange} estimating equation, and the [optimal]{.blue} choice of $\bm{A}$ minimizes the [Godambe information]{.orange}. 
...
$$
Then $\hat{\theta}_n$ is a [consistent estimator]{.blue} of $\theta$.
:::
...
- The [asymptotic normality]{.orange} of an estimator $\hat{\theta}_n$ is a stronger property than consistency, implying that the estimator is [normally distributed]{.blue} around the true value $\theta$ as the sample size increases.

- Let $\Theta \subseteq \mathbb{R}$ and let $\hat{\theta}_n$ be a sequence of estimators. Under "regularity conditions":
$$
\sqrt{n}(\hat{\theta}_n - \theta) \overset{\text{d}}{\longrightarrow} \text{N}(0, v(\theta)^{-1}), \qquad n \rightarrow \infty,
$$
where $v(\theta)^{-1}$ is the so-called [asymptotic variance]{.blue}.

- If $\hat{\theta}_n$ is the [maximum likelihood]{.orange}, then under regularity conditions $n v(\theta)$ equals the [Fisher information]{.orange} $I(\theta) = n i(\theta)$ and the [asymptotic variance]{.blue} is $i(\theta)^{-1}$. 

:::callout-tip
In regular estimation problems, the maximum likelihood estimator $\hat{\theta}_n$ is [asymptotically efficient]{.orange}, that is, for $n$ large enough, its variance attains the Cramér-Rao lower bound. Informally, 
$$
\hat{\theta}_n \:\dot{\sim}\: \text{N}(\theta, I(\theta)^{-1}).
$$
:::
...
(**A5**) There is a neighbourhood $\mathcal{N}$ of the true value $\theta_0$ within which the first [three derivatives]{.orange} of $\ell(\theta)$ exist a.s., and there exist functions $m_{rst}(y)$ such that $|\partial^3\ell(\theta)/\partial\theta_r \partial\theta _s \partial \theta_t| \le m_{rst}(y)$ and $\mathbb{E}_\theta(M_{rst}(\bm{Y})) < \infty$ for $r,s,t = 1,\dots,p$ and $\theta \in \mathcal{N}$. 
...
Under assumptions [(A1)-(A6)]{.blue}, suppose the maximum likelihood estimator $\hat{\theta}_n$ exists and is [consistent]{.orange}. Then
$$
\sqrt{n}(\hat{\theta}_n - \theta_0) \overset{\text{d}}{\longrightarrow} \text{N}(0, i(\theta)^{-1}), \quad i(\theta) = -\int\frac{\partial^2}{\partial \theta \partial \theta^T}\log{f(y \mid \theta)}\mathrm{d}y,
$$
...
- There are two natural candidates for estimating $I(\theta)$, namely $I(\hat{\theta}_n)$ and $\mathcal{I}(\hat{\theta}_n)$. These two quantities may coincide, but in general, this is not guaranteed.
...
$$
\sup_{\theta \in \Theta} \left ||Q_n(\theta) - Q(\theta)\right||_2 \overset{\textup{p}}{\longrightarrow} 0, \qquad\sup_{\theta : ||\theta- \theta_0||_2 \ge \epsilon} ||Q(\theta)||_2 > 0 = ||Q(\theta_0)||_2.
$$
...
$$
\sqrt{n}(\hat{\theta}_n - \theta) \overset{\text{d}}{\longrightarrow} \text{N}(0, v(\theta)^{-1}), \qquad v(\theta) = h(\theta)j(\theta)^{-1}h(\theta)^T,
$$
...
$$
\hat{\theta}_n \:\dot{\sim}\: \text{N}(\theta, V(\theta)^{-1}).
$$
:::
...
- Thus far, we have assumed that $\mathcal{F} = \{f(;\theta) : \theta \in \Theta\}$ is [correctly specified]{.orange}, that is, there exists a $\theta_0$ that generates the data $(Y_1,\dots,Y_n) \sim f(\cdot;\theta_0)$ and $f(\cdot; \theta_0) \in \mathcal{F}$. 
...
- The term "[robustness]{.orange}" is intentionally vague, but let us say that any statistical procedure:
  ...
  b. Small deviations from the model assumptions should impair the model only slightly
  c. Somewhat larger devitations from the model should not cause a catastrophe
...
- Case i. is sometimes called [resistence]{.orange} and relies on the notion of [influence functions]{.blue}. For instance, the median is resistent, the mean is not. Here we will consider case ii. 
...
$$
In other words, $\theta_0$ represents the parameter value that makes $f(\cdot;\theta)$ [as close as possible]{.blue} to $f_0$, although  $\mathrm{KL}(f(\cdot; \theta_0) \mid f_0) > 0.$

## Robustness and parametric estimation

<!-- - As an example, note that the [sample mean]{.blue} $\bar{Y}$ is actually robust with respect to [model misspecification]{.orange}; it can be justified, in fact, using a nonparametric argument.  -->

- Let $\mathcal{F} = \{\mu^{-1}e^{-y/\mu} : \mu \in \mathbb{R}^+ \}$ and suppose the data $Y_1,\dots,Y_n$ are iid from an [exponential]{.orange} with mean $\mu_0$. Then $\bar{Y}$ is a consistent and efficient estimator for $\mu_0$. 

- However, $\bar{Y}$ is a [robust]{.orange} estimator in the sense that if $Y_i \overset{\textup{iid}}{\sim} f_0(\cdot)$ with $f_0(\cdot) \notin \mathcal{F}$, then $\bar{Y}$ remains [consistent]{.blue} for the mean $\mu_0$ under minimal assumptions on $f_0$ (i.e. $\mu_0$ must exist).
...
:::callout-tip
An essential requirement is then that estimands have the [same interpretation]{.blue} under all the potential models. In the former example $\mu_0$ have this property of [robustness of interpretation]{.orange}. 

On the other hand, the parameters $\alpha$ and $\beta$ of a Gamma distribution do not, because they are meaningless for models other than the Gamma.
:::

## Robustness and parametric estimation

- The former example can be read under the lenses of estimating equation. The score function
$$
\ell^*(\mu) = \sum_{i=1}^n(Y_i - \mu)
$$
is an [unbiased estimating equation]{.blue} under $f_0(\cdot)$ for a broad class of models beyond $\mathcal{F}$. This ensures [consistency]{.orange} under mild conditions (cfr. @vandervaart1998, Theorem 5.9)
...
## Re-interpreting M-estimators and Z-estimators

- Results of Godambe efficiency

## Sandwich estimators

## Example: Huber estimators

- Section 10.2.2 of Casella-Berger
- See Davison, Section 8.4

## Example: linear models with misspecified variance
