...
- The range of the estimator usually coincides with $\Theta$, i.e. $\hat{\theta} : \mathcal{Y} \rightarrow \Theta$, but there are exceptions.
...
- In general, it is [not guaranteed]{.orange} that a [solution exists]{.blue} or that it is [unique]{.blue}.
...
-   Moments estimators are not necessarily the best estimators, but
    ...
    they have convergence rate $\sqrt{n}$, and they are [asymptotically
    normal]{.orange}.
...
-   Suppose also that $\mu(\theta)$ is a [one-to-one]{.blue} mapping and let $g(\mu)$ be the inverse of $\mu(\theta)$, that is $g = \mu^{-1}$. We assume that $g$ has [differentiable]{.blue} components $g_r(\cdot)$ for $r = 1,\dots,p$.
...
-   [Remark I]{.blue}: the MLE may [not exists]{.orange} and is
    ...
    $\ell(\theta)$ is differentiable, then it can be found as the solution
...
-   Let $Y_1,\dots,Y_n$ be an iid random sample from a Poisson
...
-   M- and Z-estimators are a broad class of estimators that encompass
...
- Let $Y_1,\dots,Y_n$ be an iid sample of real-valued random variables belonging to family of distributions $\mathcal{F}$ defined as
$$
...
- Data $y_1, \dots, y_{66}$ represent Simon Newcomb's measurements (1882) of the [speed of light]{.orange}. The data are recorded as [deviations]{.blue} from 24,800 nanoseconds.
...
[^7]: Some subjective Bayesians may criticize this estimator for not being "truly Bayesian" as the hyperparameters depend on the sample size $n$, making the prior data-dependent. However, here we are evaluating $\hat{p}_\text{minimax}$ from a frequentist perspective.
...
:::callout-note
Let $\Theta \subseteq \mathbb{R}$. An estimator $\hat{\theta}$ is a [best unbiased estimator]{.blue} of $\theta$ if it satisfies $\mathbb{E}_\theta(\hat{\theta}) = \theta$ for all $\theta$ (unbiasedness) and, for any [other unbiased]{.orange} estimator $\tilde{\theta}$, we have
$$
...
then 
$$
\mathbb{E}_\theta(\ell^*(\theta)) = 0,\qquad \text{implying}\qquad   \mathbb{E}_\theta(\ell^*(\theta)^2) = \text{var}_\theta(\ell^*(\theta)).
$$
:::

- Thus, the above regularity condition implies that the score function $\ell^*(\theta)$ is an [unbiased estimating equation]{.blue}. 

- The regularity condition is always satisfied, for example, in regular exponential families. 

## Cramér-Rao inequality 📖

- The Cramér-Rao theorem establishes a [lower bound]{.blue} for the variance of an estimator. Thus, if the variance of an unbiased estimator $\hat{\theta}$ attains the lower bound for all $\theta$, then $\hat{\theta}$ is [UMVUE]{.orange}. [^caveat]

::: callout-warning
#### Theorem (Cramér-Rao, Theorem 7.3.9 in @Casella2002)
Let $Y_1,\dots,Y_n$ be a sample from a joint probability measure $f(\bm{y} \mid \theta)\nu(\mathrm{d}\bm{y})$ satisfying condition [C.1]{.blue} and let $\Theta \subseteq \mathbb{R}$. Moreover, let $\hat{\theta}(\bm{Y})$ be an estimator of $\theta$ satisfying
$$
...
[^caveat]: This is essentially correct, but strictly speaking, Cramér-Rao holds only for estimators that satisfy the condition. 

## Cramér-Rao: considerations

:::callout-tip
- The interchange of the derivative under the integral sign, e.g. [C.1]{.blue}, is an important [condition]{.orange}, not merely a technical artifact of the proof.

- For example, if the [sample space]{.blue} of i.i.d. random variables $Y_i$ [depends on $\theta$]{.blue}, condition [C.1]{.blue} is violated, and the Cramér-Rao lower bound may not hold.[^uniform_example]
:::
...
In addition to the assumptions of Cramér-Rao theorem, suppose $Y_1,\dots,Y_n$ is an iid sample from $f(y \mid \theta)\mathrm{d}y$ satisfying also condition [C.2]{.blue}. Moreover, let $\hat{\theta}(\bm{Y})$ be an [unbiased]{.orange} estimator of $\theta$  with finite variance $\text{var}(\hat{\theta}(\bm{Y})) < \infty$. Then
$$
...
## Cramér-Rao: technical considerations ☠️

:::callout-tip

- The classical statement of the Cramér–Rao lower bound imposes [restrictions]{.orange} on the [estimator]{.blue} itself, which is conceptually problematic.

-	As stated, the theorem leaves open the possibility that some estimators that do not satisfy the condition may have variance below the bound.
:::

- This issue can be addressed by restricting the class of densities $f(\bm{y} \mid \theta)$, rather than on the estimators, typically a regularity condition akin to a [Lipschitz condition]{.blue}.

- Within such a restricted class, which includes regular exponential families, the Cramér–Rao bound holds for any estimator $\hat{\theta}$ and condition [C.1]{.blue} is automatically satisfied.

- See Theorem 5.15, Chap.2, @Lehmann1998 for such a refined Cramér-Rao theorem. See also note 8.6, Chap.2 for a detailed discussion.  



## Rao-Blackwell 📖

- The Rao-Blackwell theorem is a [constructive strategy]{.orange} for improving estimators that emphasizes the pivotal role of [sufficiency]{.orange} in finding UMVU estimators. 
...
- Suppose we wish to improve on an unbiased estimator $\hat{\theta}$. Then, we could consider $U = U(\bm{Y})$ such that $\mathbb{E}_\theta(U) = 0$, i.e. $U$ is an [unbiased estimator of 0]{.blue}, and let
$$
...
- Proving that an estimator $\hat{\theta}$ is uncorrelated with all unbiased estimators of 0 is very hard, limiting the practical usefulness of the former theorem.
...
- Generalizations of Rao-Blackwell theory to the multiparameter case $\Theta \subseteq \mathbb{R}^p$ are possible.
...
::: callout-tip
Let $Y_1,\dots,Y_n$ be independent random variables such that $\mu_i(\theta) = \lambda e_i$ and $V_i(\theta) = \lambda e_i$, where $e_i$ are [known constants]{.orange} e.g. representing exposure. Then the optimal weights are $w_i(\theta) = 1$ and $\hat{\theta} = \bar{Y}/ \bar{e}$. Note that $Y_i$ is [not]{.blue} necessarily [Poisson]{.blue}. 
:::
...
- When $\Sigma = \text{diag}(\sigma_1^2,\dots,\sigma_n^2)$ is [diagonal]{.orange}, i.e. in the presence of heteroscedasticity, the Aitken estimator reduces to the [weighted least squares]{.blue} estimator.
...
- Solving this estimating equation yields a linear unbiased estimator, which is
$$
...
- The [optimal]{.orange} choice of $\bm{A}$ equivalently minimizes the inverse of the [Godambe information]{.blue}, which after a few algebraic manipulations is equal to
$$
...
$$
\text{var}(\hat{\beta}) = \sigma^2 \left[(\bm{A}^T \bm{X})^{-1}\bm{A}^T\right] \left[(\bm{A}^T \bm{X})^{-1}\bm{A}^T\right]^T = V(\beta)^{-1},
$$
...
:::callout-tip
In regular problems, the maximum likelihood estimator $\hat{\theta}_n$ is [asymptotically efficient]{.orange}, that is, for $n$ large enough, its variance attains the Cramér-Rao lower bound. Informally, 
$$
...
-   Let $Y_1,\dots,Y_n$ be an iid random sample from a [Poisson]{.orange}
...
$$
for $r,s = 1,\dots,p$, that is, the [first]{.orange} and [second Bartlett identities]{.blue}. 
:::
...
Under assumptions [(A1)-(A3)]{.blue} and if $\Theta = (\theta_0,\theta_1,\dots,\theta_k)$ is [finite]{.orange}, then $\hat{\theta}_n$ exists and is unique with probability tending to $1$, and is a [consistent estimator]{.blue} of $\theta_0$.
:::
:::callout-tip
...
$$
f(y; \theta) = \frac{1}{2}\phi(y; 0, 1) + \frac{1}{2}\phi(y; \theta, e^{-2/\theta^2}),  \qquad \theta \neq 0,
$$
...
- However, the log-likelihood present [spikes]{.blue} in correspondence of the observed data  and the maximum likelihood concentrates around $0$ rather than $\theta_0$, i.e $\hat{\theta}_n$ is [inconsistent]{.orange}.

- Indeed, there are [multiple roots]{.blue} of the score equation $\ell^*(\theta)$, corresponding to the spikes. The "correct" solution close $\theta_0$ is among them, but is not identified by the data even for large $n$. 
...
- The claim is that a [clairvoyant statistician]{.blue}, with knowledge of $\theta_0$, could choose a consistent sequence of roots. In reality, it may be impossibile to choose the right solution.
...
- Informally, we say that the maximum likelihood estimator is [asymptotically efficient]{.orange} because, roughly speaking, $\text{var}_\theta(\hat{\theta}_n) \approx I(\theta)^{-1}$, the latter being the [Cramér-Rao lower bound]{.blue}.
...
$$
where the red terms are inconsequential because the maximizer $\hat{\theta}_n$ of $M_n(\theta)$ is the same. 
...
- On the other hand, we will see that $\hat{\theta}_n$ does not need to maximize $M_n(\theta)$. Indeed, it is sufficient that it [nearly maximizes]{.blue} it, in the sense that $M_n(\hat{\theta}_n) \ge \sup_{\theta \in \Theta}M(\theta) - o_p(1)$.

## Consistency for M-estimators II 📖
...
## Consistency for Z-estimators 📖

- The former theorem can also be expressed in terms of Z-estimators $Q_n(\theta)$, that is, on a set of [estimating equations]{.orange}. An example is the score function
$$
...
- The jackknife and $\hat{\theta}_\text{bc}$ are "corrective" rather than "preventive". That is, the maximum likelihood $\hat{\theta}$ is first calculated, then corrected. A practical requirement is the [existence]{.orange} of $\hat{\theta}$. 
...
## Bias reduction using Firth's correction III 📖
...
$$
which is a well-known [bias-reducing]{.orange} correction of the empirical logit. 
:::
...
- Case i. is sometimes called [resistance]{.orange} and relies on the notion of [influence functions]{.blue}. For instance, the median is resistant, the mean is not. 

## Example: Huber estimators I 📖
...
## Example: omitted variable in linear regression 📖
...
## Example: misspecified exponential model II 📖
...
- The score function is [unbiased]{.orange} for $\mu$ under $f_0$. The score and the observed information matrix are, respectively
$$
...
## Example: the probability of observing a zero 📖
...
## Example: linear models with misspecified variance I 📖
...
## Example: linear models with misspecified variance II 📖
...
- The [sandwich (White)]{.orange} estimator is instead $(\bm{X}^T\bm{X})^{-1} \bm{X}^T \bm{R}^2 \bm{X} (\bm{X}^T\bm{X})^{-1}$:
