...
- The Rao-Blackwell theorem is a classical result of inferential statistics that emphasizes the pivotal role of [sufficiency]{.orange} in finding UMVU estimators. 

:::callout-warning
#### Theorem (Rao-Blackwell, @Casella2002, Theorem 7.3.17)

Let $\tilde{\theta}(\bm{Y})$ be an [unbiased]{.orange} estimator of $\theta$, and let $S = s(\bm{Y})$ be a [sufficient statistic]{.blue} for $\theta$. Let $\hat{\theta} = \mathbb{E}_\theta(\tilde{\theta}(\bm{Y}) \mid S)$. Then $\hat{\theta}$ is an estimator such that $\mathbb{E}_\theta(\hat{\theta}) = \theta$ and
$$
...
- Rao-Blackwell theorem is a [contructive strategy]{.orange} for finding an unbiased estimator $\hat{\theta}$ that [dominates]{.orange}  another unbiased estimator $\tilde{\theta}$. However $\hat{\theta}$ is not necessarily UMVUE.
...
- The next theorem is a [partial answer]{.orange}, which is practically useful if we show that $\hat{\theta}$ attains the [Cramér-Rao]{.blue} lower bound. 

:::callout-warning
#### Theorem (@Casella2002, Theorem 7.3.19)
If $\hat{\theta}$ is a best unbiased estimator of $\theta$, then $\hat{\theta}$ is [unique]{.blue}.
:::
...
Let $\hat{\theta}$ be an unbiased estimator of $\theta$. Then $\hat{\theta}$ is UMVUE [if and only if]{.orange} $\hat{\theta}$ is [uncorrelated]{.blue} with all unbiased estimators of $0$, that is
$$
\text{cov}_\theta(\hat{\theta}(\bm{Y}), U(\bm{Y})) = 0,  
$$
for all estimators $U(\bm{Y})$ such that $\mathbb{E}_\theta(U(\bm{Y})) = 0$.
:::
...
A sufficient statistic $S = s(\bm{Y})$ for a statistical model $\mathcal{F} = \{f(\mathrm{d}y \mid \theta) : \theta \in \Theta\}$ is said to be [complete]{.blue} if [no nonconstant]{.orange} function of $S$ is [first-order ancillary]{.orange}, that is,
$$
\mathbb{E}_\theta(f(S)) = c \quad \text{for all } \theta \quad \text{implies} \quad f(S) = c \:\text{ a.s.}
$$
...
#### Theorem (Lehmann-Scheffé-Rao-Blackwell, @Casella2002, Theorem 7.3.23)
 Let $S$ be a [complete]{.orange} sufficient statistic for a parameter $\theta$, and let $\hat{\theta}$ be an unbiased estimator of $\theta$ based only on $S$. Then, $\hat{\theta}$ is [unique]{.blue} best unbiased estimator (UMVUE) of $\theta$. 
:::

- This final theorem is perhaps the most important: if an unbiased estimator of $\theta$ and a complete sufficient statistic exist, we can find the unique UMVUE.
