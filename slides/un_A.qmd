...
$$
\sqrt{n}(\hat{\theta}_n - \theta_0) \overset{\text{d}}{\longrightarrow} \text{N}(0, i(\theta_0)^{-1}), \quad i(\theta) = -\int\frac{\partial^2}{\partial \theta \partial \theta^T}\log{f(y \mid \theta)}\mathrm{d}y,
$$
...
$$
recalling that $\mathcal{I}(\theta) = - \partial/\partial \theta  \:\ell^*(\theta; \bm{y})$ is the [observed information]{.blue} matrix.
:::

- Note that, under correct specification, $\hat{V}(\hat{\theta}_n) = I(\hat{\theta}_n) + o_p(1)$.  
...
$$
\text{var}_\theta(\hat{\mu}) \approx \mathcal{I}(\hat{\mu})^{-1} = \frac{\bar{y}^2}{n}, \qquad (\text{under correct specification}).
$$
- If the model is misspecified, then we can use the [sandwich estimator]{.orange}:
$$
