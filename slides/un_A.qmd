...
[^caveat]: There are some caveats, see the additional considerations.
...
Under the same assumption of Cramér-Rao theorem, assume in addition that $Y_1,\dots,Y_n$ is an iid sample from $f(y \mid \theta)\mathrm{d}y$ satisfying also condition [C.2]{.blue}, and let $\Theta \subseteq \mathbb{R}$. Moreover, let $\hat{\theta}(\bm{Y})$ be an [unbiased]{.orange} estimator of $\theta$  with finite variance $\text{var}(\hat{\theta}(\bm{Y})) < \infty$. Then
$$
...
- The classical statement of the Cramér-Rao lower bound imposes [restrictions]{.orange} on the [estimator]{.orange} itself, which is problematic. 

- As stated, the theorem leaves open the possibility that some estimators, not satisfying the stated condition, may have variance below the bound.
:::

- This can be solved solved by imposing a fairly technical condition, similar to what is known to [Lipschitz condition]{.blue} on the considered class of densities $f(\bm{y} \mid \theta)$ rather than estimators.

- Within such a restricted class, the Cramér-Rao theorem is valid for any estimator $\hat{\theta}$. 

- See Theorem 5.15, Chap.2, @Lehmann1998 for a Cramér-Rao theorem based on the [Lipschitz condition]{.blue}.

- Refer to note 8.6, Chap.2 for @Lehmann1998 for necessary and sufficient conditions. 
