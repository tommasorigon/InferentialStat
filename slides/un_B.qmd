...
$$
\begin{aligned}
f(y;\theta) &= \prod_{j=1}^p f(y_j;\theta_j) = \prod_{j=1}^pf_j(y_j)\exp\{\theta_j y_j - K_j(\theta_j)\} \\
&= \left[\prod_{j=1}^pf_j(y_j)\right]\exp\left\{\sum_{j=1}^p\theta_jy_j - \sum_{j=1}^pK_j(\theta_j)\right\} \\
& = f_0(y)\exp\{\theta^Ty - K(\theta)\}
\end{aligned}
$$
where $f_0(y) = \prod_{j=1}^pf_j(y_j)$, $K(\theta) = \sum_{j=1}^pK_j(\theta_j)$ and the [natural parameter space]{.orange} is
$$
...
If, for every for every $\theta \in \tilde{\Theta}$, the natural observations of a [regular]{.blue} $\mathcal{F}_\text{en}^p$ are [uncorrelated]{.blue} then they are also [independent]{.orange}.
:::

- This generalizes a well-known fact of multivariate Gaussians, which are in fact an $\mathcal{F}_\text{en}^p$.
...
- Consider a $\mathcal{F}_\text{en}^p$ family, so that $f(y; \theta) = f_0(t)\exp\{\theta^T y - K(\theta)\}$. 

- Let $y = (t, u)$ be a [partition]{.blue} of the natural observations $y$, where $t(\cdot)$ has $k$ components and $u$ has $p-k$ components. Let us partition $\theta$ accordingly, so that $\theta = (\tau, \zeta)$ and
$$
f(y; \tau, \zeta) = f_0(y)\exp\{\tau^Tt + \zeta^Tu - K(\tau,\zeta)\}, \qquad (\tau,\zeta) \in \tilde{\Theta}.
$$

:::callout-warning
#### Theorem (@Pace1997, Theorem 5.6)

<!-- For an $\mathcal{F}_\text{en}^p$ with density factorized as above: -->
...
- The former result on marginal and conditional laws is not just an elegant probabilistic fact. Indeed, it has sensible inferential applications. 

- Often times, we can split the parameter vector $\theta$ into a [parameter of interest]{.blue} $\tau$ and a [nuisance parameter]{.orange} $\zeta$. We are not interested in learning $\zeta$. 

:::callout-tip
The main idea relies upon noticing that $f_{T\mid U=u}(t; u, \tau) = h_u(t)\exp\{\tau^T t - K_u(\tau)\}$ does not involve $\zeta$ and therefore we could define a [conditional likelihood]{.orange} based on $f_{T\mid U=u}$. 
:::
...
$$
and thus conditional likelihood is [discarding information]{.orange}, that is, it is neglecting $f_U(u; \tau, \zeta)$. 
...
$$
is called a [exponential family]{.orange} of order $p$, where the normalizing constant is
$$
...
:::
  ...
  - If $q = p$, then $\theta(\psi)$ must be a one-to-one mapping, i.e. a [reparametrization]{.blue}, otherwise the model is again [not identifiabile]{.orange}.
...
- Figure 4.1 of @Efron2023, Chapter 4. Three levels of statistical modeling, now with a fourth added representing curved exponential families.
...
- Without loss of generality, we can focus on the [natural parametrization]{.blue} $\theta \in \Theta \subseteq \mathbb{R}^p$ and density baseline $h(y) = f_0(y)$, meaning that $f(y;\theta) \in \mathcal{F}_\text{e}^p$ can be written as
$$
...
f_S(s; \psi) = \tilde{f}_0(s)\exp\{\theta s - K(\theta)\},
$$
for some baseline density $\tilde{f}_0(s)$, namely $f_S(s; \psi) \in \mathcal{F}_\text{en}^p$. If in addition $s(y)$ is a [one-to-one]{.orange} invertible mapping, this means $Y = s^{-1}(S)$ is just a transformation of an $\mathcal{F}_\text{en}^p$.

:::callout-tip
As in the single parameter case, a full exponential family $\mathcal{F}_\text{e}^p$ with $p = q$ in practice it leads to a [reparametrization]{.orange} of a natural exponential family $\mathcal{F}_\text{en}^p$ in a [transformed space]{.blue} $s(Y)$. 
:::
...
&=h(y)\exp\{\theta(\psi)s(y) - G(\psi)\}.
\end{aligned}
$$
where $h(y) = y^{-1}$, the [sufficient statistic]{.blue} $s(y) = (s_1(y), s_2(y)) = (\log{y}, y)$ whereas the [natural parameters]{.orange} and the cumulant generating functions are
$$
...
$$
f(s; \theta) = \tilde{h}(y)\exp\{\theta^Ts - \log{\Gamma(\theta_1)} + \theta_1\log(-\theta_2)\}, \qquad \theta \in \tilde{\Theta},
$$
...
- We define a tilted density $f(y; \theta) \in \mathcal{F}_\text{e}^2$ by considering $s(y) = (\cos{y}, \sin{y})$, i.e. be the [cartesian coordinates]{.orange} of $y$. This choice of $s(y)$ ensures the appealing property $f(y;\theta) = f(y + 2k\pi;\theta)$. 
...
$$
\exp{K(\theta)} = \frac{1}{2\pi}\int_0^{2\pi}\exp\{\theta_1\cos(y) + \theta_2\sin(y)\}\mathrm{d}y = \mathcal{I}_0(||\theta||_2),
$$
where $\mathcal{I}_\nu(\cdot)$ is known as the [modified Bessel function]{.orange} of the first kind and order $\nu$.

- It is easy to check that $K(\theta) < \infty$ for all values of $\theta \in \mathbb{R}^2$, therefore $\tilde{\Theta} = \mathbb{R}^2$. This complete the definition of what is known as the [von Mises]{.blue} distribution.

## Example: von Mises distribution II

- Instead of the [natural parametrization]{.orange} it is often convenient consider a [reparametrization]{.blue} $\psi =(\tau, \gamma)$, defined trough the one-to-one mapping
$$
...
$$
f(y; \psi) = h(y)\exp\{\theta(\psi)s(y) - G(\psi))\} = \frac{1}{2\pi \mathcal{I}_0(\tau)}e^{\tau\cos(y - \gamma)}, \qquad y \in (0, 2\pi),
$$
...
$$
f_S(s; \theta) = \frac{1}{2\pi}\exp\{\theta^Ts - \log\mathcal{I}_0(||\theta||_2)\}, \qquad s \in \mathcal{S} = \{(s_1,s_2) \in \mathbb{R}^2 : s_1^2 + s_2^2 = 1\},
$$
clarifying that $S = s(Y)$ is a random vector taking values on a [circle]{.orange} with unitary radius.  
...
- The von Mises distribution is sometimes regarded as the "Gaussian distribution for [circular data]{.blue}". To provide a concrete example, let us consider the [wind directions]{.orange} measured from the [San Giorgio metereological station](https://www.comune.venezia.it/it/content/17-san-giorgio), in Venice. 

- Measurements are recorded every [5 minutes]{.orange}, from the 14-04-2025 to 18-04-2025, for a total of $n = 1153$. The variables `wind_dir` is recorded in [degrees]{.blue}, i.e. between 0 and 360.
...
- This is a somewhat [misleading]{.orange} graphical representation of [wind directions]{.blue} evolving over time. Indeed, the "spykes" are not real: the angles 1 and 359 are, in fact, very close.
...
- A better graphical representation of [wind directions]{.blue} and [wind speed]{.orange}, using cartesian coordinates. From this windrose, it is clear the winds were coming mostly from the East. 
...
$$
from which we see that $s = \sum_{i=1}^ns(y_i) =(\sum_{i=1}^n s_1(y_i),\dots,\sum_{i=1}^n s_p(y_i))$ is the [minimal sufficient statistic]{.orange} as long as $n \ge p$, which has [fixed dimension]{.blue} $p$ whatever the sample size. 
...
$$
\ell(\theta) = \ell(\theta; s) = \theta^Ts - n K(\theta), \qquad \theta \in \tilde{\Theta},
$$
with $S = \sum_{i=1}^n s(Y_i)$ being distributed as a $\mathcal{F}_\text{en}^p$ with cumulant generating function $n K(\theta)$. 

- The [completeness]{.orange} of $S$ in exponential families is a classical result that enables the usage of Rao-Blackwell-Lehmann-Scheffé theorem for finding the UMVUE.
...
- Recall that after a reduction via sufficiency $\ell(\theta) = \theta^Ts - \tilde{K}(\theta)$. Thus, the [score vector]{.blue} is
$$
l^*(\theta) = s - \frac{\partial}{\partial \theta}\tilde{K}(\theta) = s - \mu(\theta),
$$
where $\mu(\theta) = \mathbb{E}_\theta(S)$ is the [mean value mapping]{.orange} of the sufficient statistic $S$.

- By direct calculation, this shows that the [first Bartlett identity]{.blue} holds, namely we have
$$
\mathbb{E}_\theta(l^*(\theta; S)) = \mathbb{E}_\theta(S) - \mathbb{E}_\theta(S) = \bm{0}.
$$
The [Fisher information]{.orange} is straighforward to compute, being equal to
$$
I(\theta) = \mathbb{E}_\theta(l^*(\theta)l^*(\theta)^\top) = \mathbb{E}_\theta\{(S - \mu(\theta))(S - \mu(\theta))^T\} = \text{var}_\theta(S).
$$


- Secondly, the [observed information]{.blue} is
$$
\mathcal{I}(\theta) = -\frac{\partial^2}{\partial \theta \partial \theta^T}\ell(\theta) = \frac{\partial^2}{\partial \theta \partial \theta^T}\tilde{K}(\theta) = \text{var}_\theta(S),
$$
which proves the [second Bartlett identity]{.blue} as an implication of the [remarkable]{.orange} identity $\mathcal{I}(\theta) = I(\theta)$, stronger than the usual $I(\theta) = \mathbb{E}_\theta(\mathcal{I}(\theta))$. In fact,  $\mathcal{I}(\theta)$ is non-stochastic. 
...
s - \mu(\theta) = \bm{0}.
$$
It is unique because the second derivative of the log-likelihood is such that
$$
\frac{\partial^2}{\partial \theta \partial \theta^T}\ell(\theta) = -\text{var}_\theta(S) < 0, \qquad \theta \in \tilde{\Theta},
$$
implying that $\ell(\theta)$ is [concave]{.orange} in $\theta$.
...
- As a corollary, if $\mathcal{F}_\text{en}^p$ is regular, the MLE exists and is unique [with probability one]{.blue} if and only the boundary of $C = C(\mathcal{S})$ has probability $0$. Broadly speaking, issues may occur when $S$ is a [discrete]{.orange} random variable placing positive probability at the boundary of $\mathcal{S}$. 


## Optimality properties of point estimates
