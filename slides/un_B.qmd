...
- Let $Y_1,\dots,Y_n$ be iid random vectors with density $f(y; \theta)$, where $f(y; \theta) \in \mathcal{F}_\text{e}^p$, with $f(y; \theta) = h(y)\exp\{\theta^T s(y) - G(\theta)\}$. The [likelihood]{.blue} function is
$$
L(\theta; \bm{y}) = \prod_{i=1}^n \exp\left\{\theta^T s(y_i) - G(\theta)\right\} = \exp\left\{\theta^T \sum_{i=1}^n s(y_i) - n G(\theta)\right\},
$$
...
$$
with $K(\theta) = n G(\theta)$ and for some density $\tilde{f}_0(s)$. In other words $f_S(s; \theta) \in  \mathcal{F}_\text{en}^p$.
