...
- Thus, [moments]{.blue} and [cumulants]{.orange} exist for every $\theta \in \text{int}\:{\tilde{\Theta}}$. In particular, cumulants are given by
$$
\kappa_k = \frac{\partial^k}{\partial t^k} K_\theta(t) \Big|_{t = 0} = \frac{\partial^k}{\partial t^k} K(t + \theta) - K(\theta)\Big|_{t = 0} = \frac{\partial^k}{\partial \theta^k} K(\theta), \qquad k \ge 1.
$$
- Let $Y \sim f(y;\theta)$. The first two moments of $Y$ are therefore obtained as
$$
...
$$
tightly connecting mean and variance. Moreover, we have that
$$
\frac{\partial^2}{\partial \theta^2} K(\theta) = \frac{\partial}{\partial \theta}\mu(\theta) = \text{var}_\theta(Y) > 0, \qquad \theta \in \tilde{\Theta},
$$
meaning that $K(\theta)$ is a [convex function]{.blue} and $\mu(\theta)$ is a [monotone increasing function]{.orange} of $\theta$.
