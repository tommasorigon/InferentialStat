...
$$
for some baseline density $f_0(y)$, namely $f_S(s; \psi) \in \mathcal{F}_\text{en}^1$. If in addition $s(y)$ is a [one-to-one]{.orange} invertible mapping, this means $Y = s^{-1}(S)$ is just a transformation of a $\mathcal{F}_\text{en}^1$.

:::callout-tip
The exponential family $\mathcal{F}_\text{e}^1$ is, technically, a broader definition, but in practice it leads to a [reparametrization]{.orange} of a natural exponential family $\mathcal{F}_\text{en}^1$ in a [transformed space]{.blue} $s(Y)$. 
:::
...
- The family $\mathcal{F}_{\text{ne}}^p$ is said to be [full]{.blue}, whereas a subfamily of $\mathcal{F}_{\text{ne}}^p$ with $\Theta \subseteq \tilde{\Theta}$ is [non-full]{.blue}.

## Example: multinomial distribution I 📖

- Let $Y = (Y_1,\dots,Y_{p-1}) \sim \text{Multinom}(N; 1/p,\dots,1/p)$ be a [multinomial]{.orange} random vector with [uniform probabilities]{.blue} and density 
$$
f_0(y) = \frac{N!}{y_1!\cdots y_p!}\left(\frac{1}{p}\right)^N, \qquad y = (y_1,\dots,y_{p-1}) \in \mathcal{Y} \subseteq \mathbb{R}^{p-1},
$$
where $\mathcal{Y} = \{y_1,\dots,y_{p-1} \in \{1,\dots,N\}: \sum_{j=1}^{p-1} y_j\le N\}$ and with $y_p := N - \sum_{j=1}^{p-1}y_j$. 

- The [exponential tilting]{.orange} of $f_0$ gives
$$
f(y; \theta) \propto f_0(y) e^{\theta^Ty} = \frac{N!}{y_1!\cdots y_p!}\left(\frac{1}{p}\right)^N e^{\theta_1 y_1 + \cdots + \theta_{p-1}y_{p-1}}, \qquad y \in \mathcal{Y}, \:\theta \in \mathbb{R}^{p-1}.
$$
...
$$
M_0(\theta) = \mathbb{E}\left(e^{\theta^Ty}\right) = \left(\frac{1}{p}\right)^N(1 + e^{\theta_1} + \cdots + e^{\theta_{p-1}})^N.
$$
Thus $M_0(\theta) < \infty$ for all $\theta \in \mathbb{R}^{p-1}$ and the [natural parameter space]{.orange} is $\tilde{\Theta} = \mathbb{R}^{p-1}$.
...
- In other words, the tilted density is again a [multinomial]{.blue} distribution with parameters $N$ and [probabilities]{.orange} $\pi_j = e^{\theta_j} / (1 + e^{\theta_1} + \cdots + e^{\theta_{p-1}})$. Indeed:
$$
\begin{aligned}
f(y; \theta) &= \frac{N!}{y_1!\cdots y_p!}\frac{e^{\theta_1 y_1}  \cdots  e^{\theta_p y_p}}{(\sum_{j=1}^p e^{\theta_j})^{y_1} \cdots (\sum_{j=1}^p e^{\theta_j})^{y_p}} = \frac{N!}{y_1!\cdots y_p!} \prod_{j=1}^p\left(\frac{e^{\theta_j y_j}}{\sum_{j=1}^p e^{\theta_j}}\right)^{y_j} \\
...
- The tilted density belong to a [natural exponential family]{.blue} $\mathcal{F}_\text{en}^{p-1}$ of [order]{.orange} $p-1$. 
...
:::
::: {.column width="60%"}
- @Morris1982 [JASA]{.blue} is a [seminal]{.blue} paper in the field of [exponential families]{.orange}.
