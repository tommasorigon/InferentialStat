...
- Figure 1 of @Efron2023. Three levels of statistical modeling. 
...
- We aim at building a [parametric family]{.blue} $\mathcal{F} = \{f(\cdot;\theta) : \theta \in \Theta \subseteq \mathbb{R} \}$ with common support $\mathcal{Y}$ such that $f_0$ is a special case, namely $f_0 \in \mathcal{F}$.
...
$$
Moreover, it can be shown that the following moment relationships hold:
$$
...
$$
is called an [exponential family]{.orange} of order one, where the normalizing constant is
$$
...
$$
is called a [natural exponential family]{.orange} of order $p$, $K(\theta) = \log M_0(\theta)$ and $\tilde{\Theta} = \{\theta \in \mathbb{R}^p : K(\theta) < \infty\}$ is the [natural parameter space]{.blue}. 
:::
...
- The [completeness]{.orange} of $S$ in exponential families is a classical result that enables the usage of the Rao-Blackwell-Lehmann-Scheffé theorem for finding the UMVUE.
...
- It is straightforward to verify that the [regularity conditions]{.blue} [A1–A6]{.blue} from [Unit A](un_A.html) are [all satisfied]{.blue}. Thus, Theorem 5.1 of @Lehmann1998 applies directly.
...
- This is indeed the case, the above being a [characterization](https://en.wikipedia.org/wiki/Convex_function) of [convexity]{.orange} for $K(\cdot)$, which we previously showed to have $\partial^2 /\partial \theta^2 K(\theta) > 0$ for all $\theta \in \tilde{\Theta}$. Moreover, recall that $\mu(\theta) = \partial /\partial \theta K(\theta)$.
...
- It is a [widely read]{.orange} paper and among the discussants we find, J.A. Nelder, A.C. Davison, C.N. Morris.
