...
$$
f(s; \theta) = f_0(s)\exp\{\theta^Ts - \log{\Gamma(\theta_1)} + \theta_1\log(-\theta_2)\}, \qquad \theta \in \tilde{\Theta},
$$
is a regular [natural exponential family]{.orange} of order 2, with some baseline density $f_0(s)$.
...
$$
h(y) = \frac{1}{2\pi}, \qquad y \in (0, 2\pi).
$$
...
$$
f(y; \theta) = h(y)\exp\{\theta^Ts(y) - K(\theta)\}, \qquad \theta \in \tilde{\Theta},
$$
...
- The [completeness]{.blue} of $S$ in exponential families is a classical result that enables the usage of Rao-Blackwell-Lehmann-Scheffé theorem for finding the UMVUE.

- The existence of a minimal sufficient statistic that performs a [non-trivial]{.orange} [dimensionality reduction]{.blue}, from $n$ to $p$, is a major simplification.

- This appears to be a characteristic of exponential families, with the only exception of non-regular cases. 

:::callout-warning
#### Theorem (Koopman-Pitman, @Robert1994, Theorem 3.3.3)

If a parametric family whose [support]{.orange} does [not depend]{.orange} on the [parameter]{.orange} is such that there exists a [sufficient statistic]{.blue} of [constant dimension]{.blue}, say $p$, then the family is $\mathcal{F}_\text{e}^p$.
:::
